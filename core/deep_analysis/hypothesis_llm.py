...
_RUNNER_SCHEMAS = {
    ...
    "entity_drilldown": {
        "required": ["entity_col"],
        "optional": ["date_col", "metric_cols", "dimension_cols", "peer_cols", "time_grains", "top_k"],
        "description": (
            "Адресный поиск точечных аномалий на уровне одной сущности "
            "(например один ИНН/сотрудник среди тысяч) внутри period/peer-групп. "
            "Возвращает CSV карточек с baseline, actual, deviation_pct и reason."
        ),
    },
    "outliers": {
        "required": ["value_cols"],
...
Строгие правила:
- Ты формулируешь ГИПОТЕЗЫ для будущей проверки, а НЕ выводы. rationale объясняет ЗАЧЕМ проверять, но НЕ утверждает результат.
- ЗАПРЕЩЕНО называть конкретные даты, числа, проценты или утверждать о наличии паттерна ("есть отток с ...", "повышенный риск ..."). Результатов у тебя нет.
...
def _validate_hypothesis_dict(
    ...
    for col_like in ("date_col", "value_col", "entity_col", "group_col", "category_col"):
        if col_like in params and not _cols_exist(params[col_like]):
            return None
    for list_like in ("value_cols", "metric_cols", "dimension_cols", "peer_cols", "columns"):
        if list_like in params and not _cols_exist(params[list_like]):
            return None
