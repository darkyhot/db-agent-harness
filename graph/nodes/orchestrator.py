...
class OrchestratorNodes:
    ...
    def orch_run_analytics(self, state: AgentState) -> dict[str, Any]:
        ...
        try:
            # invoke() возвращает финальный смерджнутый state подграфа целиком.
            # Раньше использовался stream() + ручное накопление final_state.update(payload),
            # но stream возвращает события по state-ключам (а не по узлам), и при
            # распаковке `list(event.values())[0]` под-словарь конкретного ключа
            # ошибочно сплющивался в верхний уровень — terял final_answer/tool_calls
            # от inner summarizer'а, из-за чего outer summarizer запускался с
            # пустым контекстом и писал "Данных недостаточно".
            final_state = sub.invoke(dict(state))
        except Exception as exc:  # noqa: BLE001
