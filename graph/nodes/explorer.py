...
class ExplorerNodes:
    ...
    def column_selector(self, state: AgentState) -> dict[str, Any]:
        ...
        system_prompt = (
            ...
            "- По умолчанию, если для сущности уже выбран *_name/*_short_name/*_full_name "
            "из dim-таблицы, числовой *_id той же сущности избыточен в select/group_by "
            "— оставляй ID только в join_keys для связи fact↔dim. "
            "Включай *_id в select/group_by ТОЛЬКО при явных триггерах в запросе "
            "пользователя: «id», «идентификатор», «код», «номер», «и id», "
            "«id и название», «код и наименование», «покажи id», «с кодом» и подобных. "
            "Примеры: «по названию ГОСБ» → select: [gosb_name], group_by: [gosb_name] "
            "(без gosb_id, он только в join_keys); «по id и названию ГОСБ» → "
            "select: [gosb_id, gosb_name], group_by: [gosb_id, gosb_name]; "
            "«по коду подразделения и его наименованию» → оба, как выше.\n"
